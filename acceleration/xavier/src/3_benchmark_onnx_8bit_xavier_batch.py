...
if __name__ == "__main__":
    ...
    for batch_size in range(2,21):
        ...
        log_runs(durations_list=dts, dest_file=f'results/batch_onnx8bit_cpu_{batch_size}.json')
    ...
    for batch_size in range(2,21):
        print(f'Batch size {batch_size}')
        dts, _ = time_inference(states=states, model=model, batch_size=batch_size)
        log_runs(durations_list=dts, dest_file=f'results/batch_onnx8bit_gpu_{batch_size}.json')
